# Language Immersion Tracker - version 1.9.4
...
The extension is completely free. It includes an optional Supabase account and cloud-sync path - sign-in UI, an upload queue, a three-month free sync trial, and self-service account deletion - in `lib/account-state.js`, `lib/supabase-auth.js`, `lib/supabase-rest.js`, `lib/cloud-contract.js`, `background.js`, and `supabase/`. Signing in is never required: local tracking, goals, history, and exports work fully without an account and are unaffected if an account is never created, is signed out of, or its free sync trial ends - only the cloud mirror stops updating. The RLS checklist in `supabase/tests/rls-checklist.md` has been run live against the production Supabase project with two real accounts (see `release/RELEASE_CHECKLIST.md`), so this release ships with the `https://*.supabase.co/*` host permission and cloud sign-in enabled. It also includes optional, consent-based product analytics (`lib/analytics-contract.js`, `lib/analytics-rest.js`, the Tracker Settings toggle in `popup.html`/`popup.js`, and a final onboarding step) - off by default, independent of cloud sync, and limited to the seven fields described in `PRIVACY.md`. See `PRIVACY.md` for exactly what cloud sync and analytics do and do not send.

## What changed in version 1.9.4

- Added self-service account deletion: a `security definer` Postgres RPC (`tracker_delete_my_account()` in `supabase/migrations/0003_self_account_deletion.sql`) that only ever deletes the caller's own `auth.users` row - never a service-role key or admin API - cascading through their `tracker_profiles`, `tracker_devices`, and `tracker_daily_totals` rows. The "Delete account" control lives only in the full dashboard's Account, Plan & Data section, styled as a destructive action and gated behind a confirmation dialog.
- Verified live against the production project with two real throwaway accounts: an unauthenticated call is rejected before it can run, a signed-in user's own account and all its cascaded rows are actually removed, an unrelated account is completely unaffected, `analytics_events` is untouched (it has no `user_id` to cascade from), and the deleted account's old access token can no longer read or write anything once its rows are gone. Both test accounts were deleted afterward - see `release/RELEASE_CHECKLIST.md`.
- Added a final, optional onboarding step for analytics consent (off by default, same wording and field list as the Tracker Settings toggle) so new users see the choice once during setup instead of only in Settings.
- Updated `PRIVACY.md` to describe account deletion as self-service rather than support-only.
