(function exposeTrackerAnalytics(globalScope) {
  ...
  function analyzePeriod(allRecords, { languageCode = "ja", days = 30, reference = new Date(), dailyGoalMinutes = 60 } = {}) {
    ...
    return {
      ...
      goalDays,
      trend: trend(currentRecords, { startKey: range.startKey, endKey: range.endKey, points: Math.min(days, 60) }),
      previousTrend: trend(previousRecords, { startKey: range.previousStartKey, endKey: range.previousEndKey, points: Math.min(days, 60) }),
